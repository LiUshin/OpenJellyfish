# OpenJellyfish 语音前台插件(LiveKit)

OpenJellyfish 之上的**实时语音上层框架**。与任务引擎解耦:对话进程(本 Worker)
...
浏览器 ──WebRTC──> LiveKit Server(SFU) <──音轨── 语音 Worker(本插件)
                                                      │ 桥接令牌(读自参与者 metadata)
                                                      ▼
                              OpenJellyfish Core: /api/voice/live/{session,delegate}
                                                      │
                                                      ▼
                                      OpenJellyfish agent(任务引擎,/api/chat 同源)
```
