...
export default function RuntimeChoiceFields({ value, onChange, profiles, deepModels = [] }: {
  value: RuntimeChoice; onChange: (choice: RuntimeChoice) => void; profiles: RuntimeProfile[];
  deepModels?: { id: string; name: string }[];
}) {
  ...
  return <Space direction="vertical" style={{ width: '100%' }}>
    ...
    {value.runtime !== 'deepagents' ? <>
      ...
      <Typography.Text type="secondary">支持原生网页搜索、图片和文件输入；生图能力取决于客户端和账号。同一连接内可逐条切换模型。</Typography.Text>
