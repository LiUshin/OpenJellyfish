# OpenJellyfish 文件系统架构

> 本文档完整描述 OpenJellyfish 在 **Local 模式** 下的物理目录结构、各文件
...
│
├── tasks/                        ← Admin 定时任务（v2 文件树）
│   └── {root_task_id}/           ← _meta.json + runs/{run_id}.jsonl（schema 见 §6）
│
...
### 3.1 Admin 对话 — `users/{uid}/conversations/{conv_id}/`

**布局**（2026-04-23 JSONL 改造后）：
...
**`messages.jsonl` 每行一条消息**（与旧 `messages[]` 元素同 schema）：

```jsonc
{"role":"user","content":"如何创建和管理 Subagent？...","timestamp":"...","attachments":[{"type":"image","filename":"wx_abc123.jpg","path":"images/wx_abc123.jpg"}]}
{"role":"assistant","content":"根据我的系统配置...","timestamp":"...","blocks":[{"type":"thinking","content":"..."},{"type":"text","content":"..."},{"type":"tool","name":"read_file","args":"...","result":"...","done":true}]}
```
...
### 3.2 Consumer 对话 — `users/{admin}/services/{svc}/conversations/{conv_id}/`

**布局**（与 admin 对称，JSONL）：

```
conversations/{conv_id}/
├── meta.json          ← {id, title, created_at, updated_at, message_count, source?}
├── messages.jsonl     ← 每行一条消息
├── generated/         ← Consumer 侧 AI 生成物
│   ├── images/  audio/  videos/
├── query_appendix/    ← 用户上传附件
└── .legacy.json       ← 懒迁移备份（旧 messages.json）
```

**`meta.json` 额外字段**：`source` 在创建时写入（`web` / `api` / `wechat`），用于 Service 使用情况面板。

**消息行 schema** 与 §3.1 相同（含 `blocks` / `attachments` / `tool_calls` 兼容字段）。

> Consumer 的 `query_appendix/` 与 admin 同级于 `messages.jsonl`，都通过
> `save_consumer_attachment()` / `get_consumer_attachment_dir()` 访问。
...
├── tasks/                         ← Service 级定时任务（agent only，v2 文件树）
│   └── {root_task_id}/            ← _meta.json + runs/{run_id}.jsonl
...
## 6. 定时任务（Scheduler）— v2 文件树

### 6.1 目录布局

```
users/{uid}/tasks/
└── {root_task_id}/                    ← task_* 或 stask_* 作路径段（非文件名）
    ├── _meta.json                     ← 任务元数据 + runs[] 摘要
    ├── runs/
    │   └── {run_id}.jsonl             ← 步骤日志（append-only）
    └── {child_task_id}/               ← spawn 子任务（可嵌套）
        ├── _meta.json
        └── runs/...
```

Service 任务对称路径：`users/{admin}/services/{svc}/tasks/{root_id}/...`

v1 扁平 `task_*.json` 在首次访问时 **lazy 迁移** 到上结构（`scheduler_tree._migrate_if_needed`）。

### 6.2 `_meta.json` 核心字段（节选）

```jsonc
{
  "id": "task_e2a9a525",
  "parent_task_id": null,           // spawn 子任务指向父 id
  "root_task_id": "task_e2a9a525",
  "spawn_chain": ["task_e2a9a525"], // root → … → self
  "spawn_depth": 0,
  "children_count": 2,
  "descendants_summary": "...",     // L3 子代摘要 LRU 1500 字
  "schedule_type": "cron",
  "schedule": "0 9 * * *",
  "task_type": "agent",
  "task_config": {
    "prompt": "...",
    "model": "anthropic:claude-sonnet-4-5-20250929",  // 可选
    "capabilities": ["web"]
  },
  "tz_offset_hours": 8,
  "next_run_at": "...",
  "runs": [ { "run_id": "...", "status": "success", "output": "..." } ]  // 摘要，无 steps
}
```

完整 script/agent `task_config` 字段与 v1 兼容；步骤详情在 `runs/{run_id}.jsonl`。

### 6.3 Admin vs Service 任务

| 区别 | Admin | Service |
|------|-------|---------|
| 路径 | `users/{uid}/tasks/` | `users/{uid}/services/{svc}/tasks/` |
| ID 前缀 | `task_` | `stask_` |
| `task_type` | `script` \| `agent` | 仅 `agent` |
| Agent | `create_user_agent` | `create_consumer_agent(channel="scheduler")` |

### 6.4 调度循环（HeapScheduler）

```
main.py startup
  └─→ HeapScheduler.start()
        ├─ min-heap [(next_run_unix, task_path)]
        ├─ asyncio.Event 唤醒（create/update/spawn/delete/run-now）
        ├─ 每 600s 全量 rescan
        └─ SCHEDULER_MAX_CONCURRENT 信号量限制并发执行
```

`_compute_next_run` 仍按 `tz_offset_hours` 解释 cron/once；`once` 且 `schedule=""` 视为立即执行。
...
OpenJellyfish 内部存在 **6 类消息流**，`reply_to` 字段统一描述目的地：
...
| # | 触发方 | 接收方 | 入口 | 出口 | 落盘位置 |
|---|--------|--------|------|------|----------|
| 1 | 用户 (Web) | Admin Agent | `POST /api/chat` | SSE 流回浏览器 | `users/{uid}/conversations/{cid}/messages.jsonl` |
| 2 | 用户 (WeChat) | Admin Agent | iLink polling | iLink sendmessage | 同上 |
| 3 | 用户 (Web) | Consumer Agent | `POST /api/v1/chat` | SSE 流 | `users/{adm}/services/{svc}/conversations/{cid}/messages.jsonl` |
...
→ 在匹配 Service 的 `tasks/{root_id}/` 树下创建 `stask_*` 子任务元数据，`reply_to` 自动
...
| 调用方 | 注入内容 | 实现 |
|--------|----------|------|
| `_run_agent_task`（admin 定时） | 从 `conversations/{cid}/messages.jsonl` tail 读最近 N 条 | `load_recent_admin_messages` |
| `_run_service_agent_task`（service 定时） | 从 `services/{svc}/conversations/{cid}/messages.jsonl` tail 读 | `load_recent_consumer_messages` |
...
```
00:00  cron 触发 → HeapScheduler 从堆取出到期 task_path
...
00:04  agent 完成 → save_consumer_message 写入 messages.jsonl (source=admin_broadcast)
00:04  _meta.json runs[] 追加摘要 + runs/{run_id}.jsonl 写入 steps
```
...
OpenJellyfish 集成腾讯 iLink Bot（`ilinkai.weixin.qq.com`），有 **两个独立的接入栈**：
...
**JSON 配置类文件**（users.json / config.json / meta.json / `_meta.json` /
inbox.json / soul/config.json / wechat_sessions.json / api_keys.json）
**当前仍写本地磁盘**——对话正文走 `messages.jsonl` append；只有 deepagents
看到的 filesystem 内容才走 S3 后端。
...
Admin 创建 cron 任务 → 微信群发
  → users/{adm}/tasks/{root_id}/.../_meta.json              [admin 任务树]
  → users/{adm}/services/{svc}/tasks/{root_id}/...          [service 任务树]
  → 触发后写 wechat_send step 到 runs/{run_id}.jsonl
...
| `users/{uid}/services/{svc}/conversations/{cid}/meta.json` + `messages.jsonl` | 同 admin；meta 可含 source | §3.2 |
| `users/{uid}/tasks/{root_id}/_meta.json` | schedule_type/task_type/reply_to/spawn_*/runs[] | §6.1 |
| `users/{uid}/services/{svc}/tasks/{root_id}/_meta.json` | admin_id/service_id/runs[] | §6.2 |
