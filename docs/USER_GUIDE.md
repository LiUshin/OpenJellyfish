...
- **品牌区**：OpenJellyfish Logo + 名称（点击可折叠侧栏，宽度从 240px → 64px）
...
在 **设置 → 通用** 中开启 **YOLO 模式** 后，Admin 对话中的 HITL 审批（`write_file` / `edit_file` / `propose_plan` 等）将**自动批准**，不再弹出审批卡片。
...
查看本账号 LLM Token 用量（按模型 / 供应商 / Service / API Key / 渠道 / 日期拆分）。数据来自 `users/{uid}/llm_usage/` 按月 JSONL 记录。
...
管理定时执行的自动化任务，分为 **管理员任务** 和 **服务任务** 两个 Tab。v2 采用**堆驱动调度**（最小堆 + 事件唤醒，精度约 1 秒）与**文件树存储**（`tasks/{root_id}/{child_id}/_meta.json`）。
