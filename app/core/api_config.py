...
Design: each Admin picks a **credential source** per provider:
  - ``platform`` — deployment env / 超管 Key（忽略该 Admin 自己填的）
  - ``user``     — Admin 设置页自己的 Key（不回落 env）

Unset source is inferred: has personal key → user, else platform
(see ``user_api_keys.get_credential_source``).

Within a single source, capability-specific env (IMAGE_*) still overrides
provider env defaults when source=platform; when source=user, capability
user fields override provider user fields.
"""
...
def _get_user_keys(user_id: Optional[str]) -> dict:
    """Load user key dict (always a dict; empty strings when unset)."""
    if not user_id:
        return {}
    try:
...
def resolve_credential_source(
    provider: str, user_id: Optional[str] = None
) -> CredentialSource:
    """``platform`` or ``user`` for this admin + provider."""
    if not user_id:
        return "platform"
    try:
        from app.core.user_api_keys import get_credential_source
        src = get_credential_source(user_id, provider)
        return "user" if src == "user" else "platform"
    except Exception:
        return "platform"
...
def get_api_config(capability: str, user_id: Optional[str] = None) -> Tuple[str, str]:
    ...
    Respects per-provider credential source (openai for image/tts/…).
    """
    ...
    user_keys = _get_user_keys(user_id) if source == "user" else {}
    ...
    if not key:
        if source == "user":
            raise RuntimeError(
                f"凭据来源为「我的 Key」，但未配置 {prefix} / {provider} 用户 Key"
            )
        raise RuntimeError(
            f"未配置 {prefix}_API_KEY 或 {provider.upper()}_API_KEY（平台环境变量）"
        )
...
def get_openai_llm_config(user_id: Optional[str] = None) -> Tuple[str, Optional[str]]:
    ...
    source = resolve_credential_source("openai", user_id)
    key = ""
    base = ""
    if source == "user":
...
def get_anthropic_llm_config(user_id: Optional[str] = None) -> Tuple[str, Optional[str]]:
    ...
    source = resolve_credential_source("anthropic", user_id)
    key = ""
    base = ""
    if source == "user":
        user_keys = _get_user_keys(user_id)
...
def has_provider(provider: str, user_id: Optional[str] = None) -> bool:
    ...
    if source == "user":
        user_keys = _get_user_keys(user_id)
        return bool(user_keys.get(f"{provider}_api_key", ""))
    key = _get_provider_key(provider)
...
def get_provider_credentials(
    ...
    if provider == "kimi":
        if source == "user":
            key = user_keys.get("kimi_api_key", "")
            base = (user_keys.get("kimi_base_url", "") or _KIMI_DEFAULT_BASE).rstrip("/")
        else:
            key = os.getenv("KIMI_API_KEY", "") or os.getenv("MOONSHOT_API_KEY", "")
            base = (
                os.getenv("KIMI_BASE_URL", "")
                or os.getenv("MOONSHOT_BASE_URL", "")
                or _KIMI_DEFAULT_BASE
            ).rstrip("/")
        return {"api_key": key, "base_url": base}
    ...
    if provider == "doubao":
        if source == "user":
            return {
                "access_key": user_keys.get("doubao_access_key", ""),
                "secret_key": user_keys.get("doubao_secret_key", ""),
                "region": user_keys.get("doubao_region", "") or _DOUBAO_DEFAULT_REGION,
            }
        return {
            "access_key": os.getenv("VOLC_ACCESSKEY", ""),
            "secret_key": os.getenv("VOLC_SECRETKEY", ""),
            "region": os.getenv("VOLC_REGION", _DOUBAO_DEFAULT_REGION),
        }

    if provider == "bedrock":
        if source == "user":
            return {
                "api_key": user_keys.get("bedrock_api_key", ""),
                "region": user_keys.get("bedrock_region", "") or _BEDROCK_DEFAULT_REGION,
            }
        return {
